...
class LoadPetSlices(Dataset):
    def __init__(self, root_dir = r"", axis = "x", load_adj = 8, seed = 1, out_size = 192, max_samples = None) -> None:
        super().__init__()
    ...
    def process_nii_file(self, data, mask):
        ...
        # Handle cases where data dimensions are smaller than target size
        if data.shape[0] < self.out_size or data.shape[1] < self.out_size:
            # Use interpolation to resize if data is smaller than target
            from scipy.ndimage import zoom
            zoom_factors = [self.out_size / data.shape[0], self.out_size / data.shape[1], 1]
            data = zoom(data, zoom_factors, order=1)
            new_data[:,:,:data.shape[2]] = data[:,:,:data.shape[2]]
        else:
            # Use center crop if data is larger than target
...
def load_data(batch_size, root_dir = r"", axis = "x", load_adj = 8, seed = 1, out_size = 192, max_samples = None, num_workers=4, pin_memory=True):
    dataset = LoadPetSlices(root_dir = root_dir, axis = axis, load_adj = load_adj, seed = seed, out_size = out_size, max_samples = max_samples)
    loader = DataLoader(dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers, pin_memory=pin_memory, drop_last=True, persistent_workers=True, prefetch_factor=4)
    while True:
        yield from loader


class LoadTestData():
    def __init__(self, root_dir = r"", axis = "z", load_adj = 8, seed = 1, out_size = 192, only_ids = None) -> None:
        ...
        if only_ids is not None:
            wanted = {str(cid).rstrip("_") for cid in only_ids}
            pairs = [
                (name, cid) for name, cid in zip(self.file_names, self.ids)
                if cid.rstrip("_") in wanted
            ]
            if len(pairs) != len(wanted):
                raise ValueError(
                    f"only_ids matched {len(pairs)} of {len(wanted)}: {sorted(wanted)}")
            pairs.sort(key=lambda item: item[1].rstrip("_"))
            self.file_names = [name for name, _ in pairs]
            self.ids = [cid for _, cid in pairs]
        else:
            order = sorted(range(len(self.ids)),
                           key=lambda i: self.ids[i].rstrip("_"))
        ...
        for i in tqdm(range(len(self.ids))):
            ...
            nac = nac_nii.get_fdata()
            mask = get_mask(nac)
            self.original_z.append(nac.shape[2])
            self.original_shape.append(nac.shape)
            self.affines.append(nac_nii.affine)
    ...
    def get_original_shape(self, idx = None):
        if idx == None:
            idx = self.idx
        return self.original_shape[idx]

    def get_affine(self, idx = None):
        if idx == None:
            idx = self.idx
        return self.affines[idx]
...
class LoadValData():
    def __init__(self, root_dir = r"", axis = "z", load_adj = 8, seed = 1, out_size = 192, max_samples = None) -> None:
